...
object SparqlConstructToModelFlowBuilder {
  ...
  val `rdf:subject` = vf.createIRI(NamespaceConstants.RDF, "subject")
  val `rdf:predicate` = vf.createIRI(NamespaceConstants.RDF, "predicate")
  val `rdf:object` = vf.createIRI(NamespaceConstants.RDF, "object")
  val `rdf:graph` = vf.createIRI(NamespaceConstants.RDF, "graph")
}
...
trait SparqlConstructToModelFlowBuilder extends SparqlClientHelpers {
  ...
  def sparqlModelConstructFlow(
    ...
    Flow.fromGraph(GraphDSL.create() { implicit builder =>
      ...
      converter ~> pooledHttpClientFlow(endpoint) ~> broadcastConstructHttpResponse ~> responseToModelFlow ~> resultZipper.in0
  ...
  val responseToModelFlow: Flow[(Try[HttpResponse], SparqlRequest), SparqlResult, _] = Flow[(Try[HttpResponse], SparqlRequest)]
    .flatMapConcat {
      case (Success(HttpResponse(StatusCodes.OK, _, entity, _)), _) =>
        entity.withoutSizeLimit().getDataBytes().zip(Source.single(entity.contentType))
    }
    .map( x => Rio.parse(x.first.iterator.asInputStream, "", toRdfFormat(x.second)))
    .flatMapConcat( m => Source.single[Model](m).via(deReifyConstructSubGraph()))
    .map(SparqlModelResult)

  def toRdfFormat(contentType: ContentType): RDFFormat = {
