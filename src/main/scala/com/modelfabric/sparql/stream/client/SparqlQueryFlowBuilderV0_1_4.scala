...
trait SparqlQueryFlowBuilderV0_1_4 extends SparqlClientHelpers {
  ...
  private def resultSetToMappedResult(resultSet: (ResultSet, SparqlRequest))(
    ...
    resultSet match {
      case (r, SparqlRequest(SparqlQuery(_,_,mappedType: MappedQuery[_],_,_,_,_,_,_,_))) =>
        mappedType.mapper
          .map(r)
