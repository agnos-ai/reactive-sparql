...
import org.modelfabric.sparql.stream.client.{GraphStoreRequestFlowBuilder, HttpEndpointFlow, SparqlRequestFlowBuilder}
import org.modelfabric.sparql.util.{HttpEndpoint, RdfModelTestUtils}
  ...
  "The Akka-Streams Graph Store Protocol Client" must {
    ...
    "9. Load TURTLE file from an http server using a URL into a named graph" in {
      ...
      def serveFileViaHttp(serverEndpoint: HttpEndpoint, rootFolder: String): Future[ServerBinding] = {
        import org.modelfabric.sparql.stream.client.SparqlClientConstants._
        val route =
          get {
